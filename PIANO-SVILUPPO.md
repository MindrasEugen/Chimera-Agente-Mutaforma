...
- **Blocchi `shell` multi-comando: `cd` non condiviso tra i comandi del blocco** — ✅ risolto (2026-08-27, scoperto nella stessa sessione di test del bug precedente, ma distinto). Osservato in uso reale: un blocco `` ```shell ``` `` con `cd testA` + scrittura file (riuscito), poi `cd ..\testB` + scrittura file — il secondo `cd` falliva con "Impossibile trovare il percorso specificato", ma il comando di scrittura successivo veniva comunque riportato `✅`, mentre il file non compariva né in `testA` né in `testB`.
  - **Causa**: il fix precedente esegue ogni riga del blocco con una `execAsync()` separata (necessario per la verifica individuale), ma ogni chiamata è un processo `cmd.exe` indipendente che riparte sempre dalla `cwd` fissa passata nelle opzioni (`process.cwd()`), non da dove un `cd` precedente l'ha lasciata — comportamento normale di `child_process.exec()`, non un bug di `cmd.exe`.
  - **Fix** (Opzione B): `executeCommands()` ora tiene traccia esplicita della working directory del blocco in una variabile locale (`currentCwd`, inizializzata a `process.cwd()`). Un comando che matcha `cd [/d] <path>` non viene più eseguito come comando shell reale: il path (con virgolette rimosse) viene risolto rispetto a `currentCwd` e verificato con `fs.statSync` — se è una cartella esistente, `currentCwd` viene aggiornata e il comando riportato `✅`; altrimenti riportato `❌` con lo stesso messaggio "Impossibile trovare il percorso specificato" e `currentCwd` resta invariata (esattamente il comportamento reale di un `cd` fallito). Tutti i comandi successivi del blocco (non-`cd`) vengono eseguiti con `cwd: currentCwd` invece della `cwd` fissa. La verifica individuale per-comando introdotta dal fix precedente resta invariata.
  - **Verificato**: (1) scenario originale ricreato — `cd` in `testA`, scrittura file, `cd ..\testB` (relativo), scrittura file, `cd ..\testC` (relativo), scrittura file: report mostra `✅` per ogni riga inclusi i tre `cd`, e controllo diretto del filesystem conferma `file.txt` presente in tutte e tre le cartelle (non solo in `testA`); (2) scenario con `cd` verso cartella inesistente in mezzo al blocco: report mostra correttamente `❌ cd ..\nonexistent` con il percorso risolto, e il comando successivo (`type nul > file2.txt`) viene eseguito — onestamente — nella cartella precedente ancora attiva (`testA`), risultato confermato sul filesystem reale, non dichiarato in una cartella sbagliata.
  - **Test automatico permanente**: `tests/test-shell-multi-comando.js` (eseguibile con `node tests/test-shell-multi-comando.js`, nessuna dipendenza da un modello AI o da input interattivo — chiama direttamente `executeCommands()` con `confirmFn`/`confirmWordFn` mockate). Copre i due scenari verificati manualmente sopra (Caso 1: `cd` relativi in cascata su tre sottocartelle; Caso 2: `cd` verso cartella inesistente a metà blocco, verifica che il fallimento sia riportato comando per comando — collegato anche al bug precedente sul falso successo — e che il comando successivo resti onestamente nella cwd valida), verificando sia il testo di report riga per riga sia il filesystem reale, con pulizia automatica delle cartelle di prova a fine esecuzione (anche in caso di eccezione). Da rilanciare dopo ogni futura modifica a `executeCommands()`. Verificato: entrambi i casi passano, exit code 0, nessun residuo lasciato in `%TEMP%`.
